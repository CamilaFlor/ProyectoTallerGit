...
void loop() {
  ...
  if (valorsensorIzq == enlinea && valorsensorMed == enlinea && valorsensorDer == enlinea ) {
    ...
    //Le avisa a processing que se paró el robot - "bifurcacion"
    Serial.print("bifurcacion");

    //---------llega indicacion x blue desde processing
    if ( ModuloHC05.available() ) { //si llega algo por blue desde processing
